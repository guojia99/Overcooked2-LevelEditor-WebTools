...
namespace CustomStub
{
    ...
    public class RandomCrate : MonoBehaviour
    {
        ...
        private const string Version = "v8(" + StubVersion.Value + ")";

        /// <summary>场景自愈载体 tag 前缀（EntryPoint.HealScene 按此挂载）。
        /// 格式 v3：RandomCrate|&lt;iconGuid&gt;|&lt;w1,w2,...&gt;|air=&lt;w&gt;。</summary>
        public const string TagPrefix = "RandomCrate|";

        /// <summary>诊断级日志（v12 起默认关闭，见 StubLog 类头）。本组件的日志
        /// 绝大多数是「验证期需要、稳态期冗余」：初始化明细、反射自检、等待心跳、
        /// 候选加载/注册、问号绘制，以及【每次取出 2 条】的掷骰/取出记录——后者
        /// 在多箱子高频关卡里是每秒数条 × (反射 Invoke + 同步写盘)。告警一律常开。</summary>
        ...
        private IEnumerator RunInner()
        {
            ...
            while (true)
            {
                ...
                //
                // v12：改用 LoadAssetAsync——同步 LoadAsset 会在主线程一次性反序列化
                // 全部候选（多候选箱子 = 开局一次数十 ms 的卡顿）。本循环已在协程里，
                // 且「加载失败 → 跳过 / 无可用候选 → 1s 后重来」的重试语义原样保留。
                // 注意 C#4 不允许在带 catch 的 try 里 yield，故把 try 拆成
                // 「发起请求」与「取结果」两段，yield 夹在中间。
                ...
                for (int i = 0; i < m_itemSOs.Length; i++)
                {
                    ...
                    try
                    {
                        bundle = GameApi.GetAssetBundle(so.bundleName);
                        if (bundle != null)
                            request = bundle.LoadAssetAsync<GameObject>(so.assetPath);
                    }
                    catch (Exception ex)
                    {
                    ...
                    if (request == null)
                    {
                        LogWarn("[RandomCrate] 食材加载请求创建失败 " + so.assetPath + ": " + name);
                        continue;
                    }
                    ...
                    {
                        LogWarn("[RandomCrate] 食材加载结果读取异常 " + so.assetPath + ": " + ex.Message);
                        continue;
                    }
                    if (prefab == null)
                    {
