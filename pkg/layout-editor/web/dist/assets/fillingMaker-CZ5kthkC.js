import{Q as w,U as T,n as k,w as P,a0 as m,W as M,a1 as A,Y as S,z as x,a2 as q,a3 as j}from"./version-BY9wjIDF.js";import{openRecipeModelPreview as U}from"./recipeModelPreview-Bz0UhbvP.js";import{a as W}from"./foodIconChain-GPcILpbK.js";import{renderRecipeForm as Z}from"./customRecipes-W9W-tMxd.js";import"./modelUnits-BbgM9XXv.js";const z="burger_filling";function l(s){return String(s??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function p(s,d=!0){const o=document.getElementById("fm-status");o&&(o.textContent=s,o.classList.toggle("err",!d),o.classList.toggle("ok",d&&s.length>0))}function E(s){return s.assetPath.replace(/\\/g,"/").includes("/commonW2/")}function N(s){return(s.score??0)>0?!1:E(s)?(s.subcategory??"")==="filling":!0}async function Q(s){document.body.classList.add("manage-bg");const d=w(),o=d.page==="filling-maker"?d.setId??"":"";let u=[];try{u=await T()}catch{}if(u.length===0){s.innerHTML=`${k("custom-recipes")}
      ...
      <div class="manage-content"><div class="m-block"><h3>没有可用关卡集</h3>
        ...
        若是桥接未启动，请在 Unity 中 Tools → Layout Editor → 启动服务。</p></div></div>`,P();return}const I=o!==""&&u.some(t=>t.setName===o);let i=I?o:u[0].setName;const B=I?d.fillingId??"":"",L=m(i,B||void 0);location.pathname!==L&&history.replaceState(null,"",L);function f(t){location.pathname!==t&&history.pushState(null,"",t)}function C(){const t=w();return t.page==="filling-maker"&&!!t.fillingId}let h=[];async function v(){s.innerHTML=`
      ${k("custom-recipes")}
    ...
    `,P();const t=document.getElementById("fm-content");let c=[];try{M("加载夹心…"),c=await A(i)}catch(e){t.innerHTML=`<div class="m-block"><h3>加载失败</h3><p class="muted">${l(e.message)}</p><p class="muted">请确认 Unity 编辑器已启动且桥接服务运行中。</p></div>`,S();return}finally{S()}const a=c.filter(e=>!E(e)&&N(e)),n=c.filter(e=>E(e)&&N(e));h=a;const r=(e,b)=>{let $;try{$=x(q(e),{iconSrc:()=>W({id:e.id,assetPath:e.assetPath,iconState:e.iconState,iconFallbackIds:e.ingredients??[],isCustom:!0})})}catch(R){$=`<div class="m-card"><h3>${l(e.nameZh)}</h3><p class="muted">卡片渲染失败：${l(R.message)}</p></div>`}const H=e.hasModel||!!e.hasModelSO||!!e.previewable?'<span class="bm-badge ok">有模型</span>':'<span class="bm-badge warn" title="没有模型的夹心在汉堡里那一层看不见，且不会出现在汉堡工作台候选中">⚠ 无模型</span>';return`
        <div class="cr-card-wrap">
          <div class="cr-card-inner">${$}</div>
          <div class="cr-card-foot">
            ${b?'<span class="cr-cat-tag">共享库（只读）</span>':`<span class="cr-cat-tag">${l(e.category)}</span>`}
            ${H}
            <span style="flex:1"></span>
            ${e.previewable?`<button class="m-btn small" data-fm-preview="${l(e.assetPath)}" data-fm-name="${l(e.nameZh)}" title="3D 模型在线预览">👁</button>`:""}
            ${b?"":`<button class="m-btn small" data-fm-edit="${l(e.assetPath)}">编辑</button>`}
            ${b?"":`<button class="m-btn small danger" data-fm-del="${l(e.assetPath)}">删除</button>`}
          </div>
        </div>`};t.innerHTML=`
