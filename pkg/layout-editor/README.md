...
- **食材分组**：每个菜谱按烹饪步骤分组成组展示，有步骤的组底部居中显示锅具图标（`icons/catalog/<utensil>.png`）与步骤名（`cooking-steps.json`）；无锅具的食材（汉堡胚、生菜等）合并为生食材组。分组算法见 `recipes.json` 新增字段 `cookingGroups`（`LayoutEditorRecipeKnowledge.ComputeCookingGroups` 与 `build-catalog.mjs` 的 `computeCookingGroups` 镜像实现）：煎锅仅覆盖肉排类中间产物（面包胚保持生），其余烹饪步骤整锅入组；**面粉系（非蛋糕族）如烧麦/松饼：全部食材一起进搅拌碗，最终锅具作空标记组**，前端 `mergeFinalMarkers` 合并为一格双图标（烧麦 = 搅拌碗 + 蒸笼）——依据是 bundle 实测的原版组成（`SteamedSpecial_Carrot ← MixedFlourCarrot(面粉+胡萝卜) → Steamer`）；蛋糕/布丁族仍是「面粉鸡蛋搅拌 + 其余进烤箱」。自定义菜谱若为**组装型（Composite）且无整体烹饪步骤**，则按其直接组成（子菜谱）分组成组——子菜谱以自身烹饪步骤成组并展开叶食材，普通食材归生组（`compositionIds` 字段 + `deriveCompositionGroups` 前端镜像）。旧数据缺字段时前端自动回退推导（`web/src/recipeGroups.ts`）。
- **锅具自动填充**：`computeUtensilIngredientFill` + `applyUtensilIngredientFill`（锅具管理页「按菜谱自动填充」、菜谱弹窗「自动补全道具」、测试布局三入口共用）。两条要点：① 「搅拌后整锅下终锅」的菜谱把终锅标记为 **clear = 写空列表**——`PseudoPrefabCookingUtensil.Setup` 只在 `allowedIngredientSOs` 非空时才整表替换原版 lookup，而原版 `SteamerPrefabLookup`（鱼 + 三种烧麦面团）/ `FryableObjectsLookup`（肉/虾/鱼/蘑菇 + 松饼面糊）本就够用，写生食材反而把这道菜做废；② **容量按「单道菜谱一次装几件」算**（多菜谱取最大、下限 1），与 bundle 实测原版一致：蒸笼/煎锅 1、搅拌碗 2~4、汤锅 3、烤串 3。
...
- **编辑/新建表单**：
  ...
  - 模型区：复用已有模型 / 上传图标 / **上传 3D 模型（FBX/OBJ，OBJ 可同时选择 `.mtl` 与贴图文件一并上传；主模型统一命名 `<recipeName>.fbx|.obj`，贴图保留原名写入 models 目录供 Unity 导入关联）**；
  - **模型尺寸与单位**：尺寸/位置输入以 **cm** 为单位（默认 4 位小数）。换算关系（实测确认）：**1 模型单位 = 1 Unity 单位 = 1 m = 100 cm**（Blender 导出 OBJ/FBX 经 Unity `useFileScale=1` 导入为 1:1）。游戏真实盘子直径 100 cm；官方参考菜（如蛋炒饭）足迹约 82~87 cm，自动适配默认目标 = **85 cm**（盘子）/ 37 cm（杯子，并抬升 30 cm 到杯中）；OBJ 上传时自动改写 `mtllib`/`map_*` 引用为落盘文件名，FBX 改写内部贴图引用名，Unity 导入即可自动链接贴图；
  - **3D 在线预览（虚拟包围盒 + 容器标的物）**：列表卡片「👁 预览」或编辑页「预览当前模型」按钮，浏览器内用 three.js 渲染 FBX（自动加载内嵌贴图）/OBJ（支持 `.mtl` 材质贴图），支持旋转/平移/缩放，尺寸/位置按 cm 显示。预览中显示**参考容器标的物**（半透明、无碰撞：盘子直径 100 cm / 玻璃杯口径 69 cm，模型来自 `web/public/models/ref_plate.obj`、`ref_glass.obj`），**容器包围盒中心 = 原点 (0,0,0)**——游戏内高度/大小都以容器中心为原点配置；模型带**虚拟包围盒**（橙色线框 + 6 个面中心点，读数实时显示足迹/长宽高 cm），可选择**箱面中心作为原点**或**让某面朝下**，也可点击模型上任意点重设原点；「自动适配」按目标尺寸（盘子 85 cm / 杯子 37 cm）缩放，**位置 Y 保持 0（不自动下沉，高度手动调）**——足迹基准 = Unity 世界包围盒水平足迹（已含配置旋转，即游戏内实际尺寸），预览显示与游戏内尺寸一致（unitK 由内容尺寸反向还原计算，避免旋转导致的轴交换误差）。
  - **方向约定（Blender ↔ Unity）**：Blender 为 Z-up，Unity 为 Y-up。FBX 文件自带 `UpAxis` 与节点旋转，Blender 导出/Unity 导入会做转换，正常导出的 FBX 两端朝向一致（three.js 解析亦一致）；**OBJ 无轴向元数据**（Blender 按 Z-up 写出顶点，Unity 按 Y-up 原样导入），在 Blender 里 Z-up 摆平的模型导入 Unity 后可能侧立，用「⬇ 该面朝下」一键摆正即可。
...
#### Mac 开发 + Windows 虚拟机测试（共享目录工作流）

本仓库源码在 Mac 上编辑，Unity/测试在 Windows 虚拟机内运行（共享目录同步代码）：

- **只改前端、跑 `npm run dev` 时**：Mac 上的 Vite 无法访问虚拟机内的 `127.0.0.1:8765`，需把 `/api` 代理指向虚拟机 IP：
  ```bash
  cd layout-editor/web && VITE_DEV_PROXY_TARGET=http://<虚拟机IP>:8765 npm run dev
  ```
- **Unity 服务端监听**：`LayoutEditorHttpServer` 默认优先绑定通配前缀 `http://*:8765`（无权限时自动回退仅本机），虚拟机内需在防火墙放行 8765 端口（TCP 入站）才可被 Mac 访问。
- **完整测试在虚拟机内进行**：共享目录内 `npm install` + `npm run dev`（或直接用已提交的 `dist/`），浏览器访问 `http://localhost:8765` 即可，无需任何网络配置。
- **注意**：路径处理一律使用相对资产路径（`Assets/...`），不要引入 Mac 绝对路径。
