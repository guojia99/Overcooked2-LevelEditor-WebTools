...
namespace CustomStub
{
    ...
    internal static class PushableVoidFall
    {
        ...
        private const float ProbeIdleSeconds = 5f;
        ...
        /// <summary>复用的临时列表（v12）：原实现在 PruneStates / TickPendingRespawns
        /// 里各 new 一个 List，而这两处跑在 15Hz 的 tick 上——有 state 时就是
        /// 30 次小对象分配/秒的纯浪费。两者顺序调用且不嵌套，共用一个 scratch 安全。</summary>
        private static readonly List<Transform> s_scratch = new List<Transform>();
        ...
        // ---- 热路径异常 once-flag（用户约定：每类异常只报一次，不刷屏） ----
        ...
        /// <summary>场景切换时由 EntryPoint 调用：清掉全部跨场景缓存
        /// （v5 新增——此前 s_killPlaneColliders 一次性缓存后永不重扫，换关卡后
        /// 旧 killplane 成假 null、新关卡的永远找不到）。</summary>
        internal static void OnSceneChanged()
        {
            s_states.Clear();
            s_targets = null;
            s_nextTargetProbe = -1f;
            s_players = null;
            s_nextPlayerProbe = -1f;
            s_killPlanesReady = false;
        ...
        private static void EnsureTargets()
        {
            ...
            s_targets = list.Count == 0 ? EmptyComponents : list.ToArray();
        ...
        internal static void RegisterTarget(Component target)
        {
            ...
            if (s_targets == null)
            {
                s_targets = new[] { target };
                s_nextTargetProbe = Time.unscaledTime + ProbeSeconds;
                return;
            }
        ...
        /// <summary>玩家列表缓存（~1s；仅在有 target 的路径上被调用）。</summary>
        private static Component[] GetPlayers()
        {
            if (s_players != null && Time.unscaledTime < s_nextPlayerProbe)
                return s_players;
            var raw = GameApi.FindAll(GameApi.PlayerControlsType);
            var list = new List<Component>();
            ...
            s_players = list.ToArray();
            s_nextPlayerProbe = Time.unscaledTime + ProbeSeconds;
            return s_players;
        }
        ...
        // ---- 组件引用惰性解析（v12）----
        // 原实现每 tick（15Hz）对每个 target 重新做 GetComponent(Pushable) /
        // GetComponentInChildren(Pilot) / GetComponentInChildren<Rigidbody>() /
        // GetComponentInChildren<Collider>()，尽管 CachePotRefs 早就把它们缓存进
        // state 了却从不复用。现改为「state 里有且未失效就直接用，否则重查一次」
        // ——Unity 假 null 语义保证被销毁的引用会自动触发重查，语义不变。

        private static Component ResolvePushable(PotTrackState state, Component target)
        {
            if (state.Pushable != null)
        ...
        private static Component ResolvePilot(PotTrackState state, Component target)
        {
            if (state.Pilot != null)
                return state.Pilot;
            state.Pilot = GameApi.GetComponentInChildren(target.gameObject, GameApi.PilotMovementType);
            return state.Pilot;
        }

        private static Rigidbody ResolveRigidbody(PotTrackState state, Component target)
        {
        ...
        private static Collider ResolveCollider(PotTrackState state, Component target)
        {
        ...
        internal static void Tick()
        {
            ...
            if (!s_attachChainChecked)
            {
                ...
                StubLog.Dbg("[VoidFall] attach 反射自检: UseAttachPoints=" + (GameApi.PushableUseAttachPointsField != null)
            ...
            for (int t = 0; t < targets.Length; t++)
            {
                ...
                // wrapper/state 先行：后续组件引用一律从 state 惰性解析（v12），
                // 不再每 tick 重复做子树查找。
                ...
                var pushable = ResolvePushable(state, target);
                if (pushable == null)
                    continue;

                var pilot = ResolvePilot(state, target);
                var rb = ResolveRigidbody(state, target);
                var col = ResolveCollider(state, target);
        ...
        private static bool IsAnyoneAttachedByAttachPoints(Component pushable)
        {
            ...
            {
                // 拖拽判定链断裂 → 误判「无人拖拽」→ 锅从手里直接坠（用户实测过的坑）
                WarnOnce("attachPoints", "[VoidFall] attach-point 拖拽判定异常（退化为 parent 兜底）: " + ex.Message);
            }
            return false;
        }
        ...
        private static bool IsAttachedTo(Component pushable, Transform tr)
        {
            ...
            {
                WarnOnce("isAttachedTo", "[VoidFall] IsAttached 调用异常: " + ex.Message);
                return false;
            }
        }
        ...
        private static void EnsureRespawnVisibility(Transform wrapper, GameObject carrier)
        {
            ...
            StubLog.Dbg("[VoidFall] 重生诊断: " + wrapper.name
        ...
        private static void EndPushableSession(Component pushable)
        {
            ...
            {
                WarnOnce("sessionGet", "[VoidFall] 读取推送会话字段异常（松手流程跳过）: " + ex.Message);
                return;
            }
            ...
            if (GameApi.ServerSessionOnEndedMethod != null)
            {
                ...
                {
                    // 失败 = 交互抑制可能未解除（从此无法再抓取——旧版踩过的坑）
                    WarnOnce("sessionEnded", "[VoidFall] ServerSessionInteractable.OnSessionEnded 调用异常（交互抑制可能残留）: " + ex.Message);
                }
            }
        }
        ...
        private static void DetachAllPlayersFromPushable(Component pushable)
        {
            ...
            if (pilot != null && GameApi.PilotAssignPlayerMethod != null)
            {
                ...
                {
                    WarnOnce("assignPlayer", "[VoidFall] Pilot.AssignPlayer(null) 调用异常: " + ex.Message);
                }
            }
        ...
        private static void RestoreDetachedPlayer(Component pc)
        {
            ...
            if (GameApi.PlayerGroundCastField != null)
            {
                ...
                if (groundCast != null)
                {
                    ...
                    {
                        WarnOnce("groundCast", "[VoidFall] 玩家 GroundCast 恢复异常: " + ex.Message);
                    }
                }
            }
            ...
            if (GameApi.PlayerApplyGravityField != null)
            {
                ...
                {
                        WarnOnce("applyGravity", "[VoidFall] 玩家重力恢复异常: " + ex.Message);
                }
            }
        }
        ...
        private static bool IsLocallyControlled(Component pc)
        {
            ...
            {
                // 判定失败按本机处理（旧行为）；联机下误判会误恢复远程玩家物理
                WarnOnce("locallyControlled", "[VoidFall] IsLocallyControlled 判定异常（按本机处理）: " + ex.Message);
                return true;
            }
        }
        ...
        private static void DisposePotContents(Transform wrapper)
        {
            ...
            for (int i = 0; i < disposals.Length; i++)
            {
                ...
                {
                    WarnOnce("dispose", "[VoidFall] 倾倒锅内容物异常: " + ex.Message);
                }
            }
        }
        ...
        private static void ClearPilotGridTarget(Component pilot)
        {
            ...
            {
                WarnOnce("clearGridTarget", "[VoidFall] 清 pilot 网格目标异常: " + ex.Message);
            }
        }
        ...
        private static void ReleasePilotGrid(Component pilot)
        {
            ...
            {
                // 格子占用残留 = 坠落点附近可走格被永久占用
                WarnOnce("releaseGrid", "[VoidFall] 释放载具网格占用异常: " + ex.Message);
            }
        }
        ...
        private static void OccupyPilotGridNear(Component pilot)
        {
            ...
            {
                // 占用失败静默跳过（首次被拖动时自行修复）——但仍留一条一次性痕迹
                WarnOnce("occupyGrid", "[VoidFall] 重生点网格重新占用异常（拖动时自行修复）: " + ex.Message);
            }
        }
        ...
        private static void SetKinematic(Rigidbody rb, bool kinematic)
        {
            ...
            if (motion != null && GameApi.SetKinematicMethod != null)
            {
                ...
                {
                    WarnOnce("setKinematic", "[VoidFall] RigidbodyMotion.SetKinematic 异常（回落直写 isKinematic）: " + ex.Message);
                }
            }
