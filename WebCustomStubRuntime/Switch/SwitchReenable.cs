...
namespace CustomStub
{
    ...
    public class SwitchReenable : MonoBehaviour
    {
        ...
        private bool m_firstResetLogged;
        ...
        private IEnumerator RunInner()
        {
            if (!s_loggedSelfCheck)
            {
                ...
                StubLog.Dbg("[SwitchReenable] 反射自检: TriggerDisableScript=" + (GameApi.TriggerDisableType != null)
            ...
            StubLog.Dbg("[SwitchReenable] 绑定按钮: " + name + " → " + m_watched.gameObject.name
            ...
            while (true)
            {
                if (m_watched == null)
                {
                    StubLog.LogWarn("[SwitchReenable] 监视的按钮脚本已销毁，协程退出（重开关卡由自愈重建）: " + name);
                    yield break;
                }
                ...
                if (m_lastEnabled && !enabledNow)
                {
                    ...
                    if (m_watched != null && !m_watched.enabled)
                    {
                        ...
                        {
                            m_firstResetLogged = true;
                            StubLog.Dbg("[SwitchReenable] 首次复位触发: " + name + " → " + m_enableTrigger);
                        }
                    }
                }
