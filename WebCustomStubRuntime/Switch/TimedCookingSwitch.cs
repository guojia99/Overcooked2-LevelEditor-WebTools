...
namespace CustomStub
{
    ...
    public class TimedCookingSwitch : MonoBehaviour
    {
        ...
        /// <summary>灶台子树内是否存在任一处于加热相位的定时开关（通用查询入口）。
        ///
        /// ⚠ 本方法每次调用都会 GetComponentsInChildren（分配一个数组），
        /// 【不得放进热路径】。HotPot 原先在 IsOverBurner（20Hz × 每锅 × 每灶台）
        /// 里直调本方法，是 GC 卡顿的主因；v12 起 HotPot 改为在 ~1s 的灶台缓存刷新
        /// 里缓存开关引用、逐次只读相位字段（HotPot.IsRegionHeating，与本方法语义等价）。
        /// 新增调用点前先确认调用频率。</summary>
        ...
        private IEnumerator Drive()
        {
            ...
            StubLog.Dbg("[TimedSwitch] 绑定 CookingRegion: " + name
            ...
            while (true)
            {
                ...
                if (phaseLeft <= 0f)
                {
                    ...
                    if (!m_firstFlipLogged)
                    {
                        ...
                        StubLog.Dbg("[TimedSwitch] 首次相位翻转: " + name + " → " + (m_phaseOn ? "开" : "关"));
                    }
                }
        ...
        private void BindFlameAssets()
        {
            ...
            StubLog.Dbg("[TimedSwitch] 火焰资产: " + name
