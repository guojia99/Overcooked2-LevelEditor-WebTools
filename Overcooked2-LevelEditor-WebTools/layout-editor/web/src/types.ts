...
export interface CatalogItem {
  ...
  /** Catalog flag: decor/recipes wrapper prefab (icon from recipes/). */
  recipeDecor?: boolean;
  /** Referenced visual PseudoPrefabSO guid (from wrapper prefab). */
  recipeGuid?: string;
  bundleName?: string;
  prefabName?: string;
...
export interface AnimGroupWaypoint {
  id: string;
...
export interface AnimGroupEvent {
  id: string;
  ...
  type: "move" | "wait" | "lift" | "drop" | "rotate" | "shake" | "flash";
  /** Auto-generated by the backend when empty ("Move1".."MoveN"). */
  triggerName?: string;
  /** 时间轴绝对开始时间（秒，相对组队列启动，0.1 对齐）。事件时间区间
   *  [startTime, startTime+时长] 重叠即并行播放（烘焙为同一组合 clip）。
   *  undefined = 旧顺序数据，加载时按 delay 链自动迁移。 */
  startTime?: number;
  /** @deprecated 旧顺序模型的触发间隔（queue delays[i]）；迁移后由 startTime
   *  差值派生，仅用于迁移计算与旧档兼容。 */
  delay: number;
  /** wait 事件：停顿时长（秒，默认 1，0.1 对齐）。 */
  duration?: number;
  /** rotate 事件：旋转角度（0~360）。 */
  rotateDegrees?: number;
  /** rotate 事件：方向（"cw" 顺时针俯视 / "ccw" 逆时针，默认 cw）。 */
  rotateDirection?: "cw" | "ccw";
  /** rotate 事件：旋转用时（秒，默认 2，0.1 对齐）。 */
   ...
   *  move/rotate 通用；仅在组内最后一个时间簇的唯一事件上生效。 */
  loop?: boolean;
  ...
  dropSeconds?: number;
  /** shake 事件：抖动幅度（米，默认 0.15）——相机 localPosition 抖动的半幅。 */
  shakeAmplitude?: number;
  /** flash 事件：闪电峰值光照强度（默认 4，对齐 oc1 4-2 的 day 灯峰值）。 */
  flashIntensity?: number;
  /** flash 事件：闪电颜色（"#rrggbb"，默认淡蓝白 #DEE1FF）。 */
  flashColor?: string;
  /** flash 事件：雷声 AudioTrigger 事件名（默认 DynamicStage01Thunder，空 = 无声）。 */
  soundCue?: string;
}

export interface AnimGroupMemberOffset {
  instanceId: string;
  x: number;
  z: number;
...
export interface AnimGroupMember {
  instanceId: string;
  /** Plain-object member display name (imported levels; web display only). */
  displayName?: string;
  /** Full scene hierarchy path of the member (web shows its child items). */
...
export interface AnimGroupMemberGroup {
  id: string;
  name: string;
...
export interface AnimGroup {
  ...
  memberOffsets?: AnimGroupMemberOffset[];
  /** Members that hold still while the route moves. */
  memberStatic?: AnimGroupMember[];
  ...
  waypoints: AnimGroupWaypoint[];
  events: AnimGroupEvent[];
...
export interface LayoutDocument {
  ...
  deathInfo?: DeathInfo;
  /** 动画组（装饰 + 物品层的移动/旋转等动画；写回时烘焙为原生 Animator 动画）。 */
  animControls?: AnimControlData;
  /** @deprecated 旧键名（moveControls），仅读取兼容旧文档；写入一律用 animControls。 */
  moveControls?: AnimControlData;
...
// ---------- Button ↔ AnimGroup links（按钮/压力开关 联动动画组） ----------
