	...
	public class TouchManager : SingletonMonoBehavior<TouchManager>
	{
		...
		private void Update()
		{
			if (isReady)
			{
				...
				if (screenSize != vector)
				{
					UpdateScreenSize(vector);
				}
				if (TouchManager.OnSetup != null)
				{
					TouchManager.OnSetup();
		...
		{
			cachedTouches = new Touch[16];
			for (int i = 0; i < 16; i++)
			{
				cachedTouches[i] = new Touch(i);
			}
			mouseTouch = cachedTouches[15];
			activeTouches = new List<Touch>(16);
