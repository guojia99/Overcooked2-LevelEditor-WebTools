import{I as w,K as T,n as k,w as P,W as m,N,X as x,P as I,x as A,Y as q,Z}from"./version-DbhnSXba.js";import{openRecipeModelPreview as j}from"./recipeModelPreview-CFteY4EO.js";import{a as W}from"./foodIconChain-GPcILpbK.js";import{renderRecipeForm as D}from"./customRecipes-BDADkUkU.js";import"./main-CgY3UP_x.js";import"./modelUnits-BbgM9XXv.js";const G="burger_filling";function l(s){return String(s??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function p(s,d=!0){const o=document.getElementById("fm-status");o&&(o.textContent=s,o.classList.toggle("err",!d),o.classList.toggle("ok",d&&s.length>0))}function S(s){return s.assetPath.replace(/\\/g,"/").includes("/commonW2/")}function M(s){return(s.score??0)>0?!1:S(s)?(s.subcategory??"")==="filling":!0}async function X(s){document.body.classList.add("manage-bg");const d=w(),o=d.page==="filling-maker"?d.setId??"":"";let u=[];try{u=await T()}catch{}if(u.length===0){s.innerHTML=`${k("custom-recipes")}
      ...
      <div class="manage-content"><div class="m-block"><h3>没有可用关卡集</h3>
        ...
        若是桥接未启动，请在 Unity 中 Tools → Layout Editor → 启动服务。</p></div></div>`,P();return}const E=o!==""&&u.some(t=>t.setName===o);let i=E?o:u[0].setName;const B=E?d.fillingId??"":"",L=m(i,B||void 0);location.pathname!==L&&history.replaceState(null,"",L);function f(t){location.pathname!==t&&history.pushState(null,"",t)}function C(){const t=w();return t.page==="filling-maker"&&!!t.fillingId}let h=[];async function v(){s.innerHTML=`
      ${k("custom-recipes")}
    ...
    `,P();const t=document.getElementById("fm-content");let c=[];try{N("加载夹心…"),c=await x(i)}catch(e){t.innerHTML=`<div class="m-block"><h3>加载失败</h3><p class="muted">${l(e.message)}</p><p class="muted">请确认 Unity 编辑器已启动且桥接服务运行中。</p></div>`,I();return}finally{I()}const a=c.filter(e=>!S(e)&&M(e)),n=c.filter(e=>S(e)&&M(e));h=a;const r=(e,b)=>{let $;try{$=A(q(e),{iconSrc:()=>W({id:e.id,assetPath:e.assetPath,iconState:e.iconState,iconFallbackIds:e.ingredients??[],isCustom:!0})})}catch(R){$=`<div class="m-card"><h3>${l(e.nameZh)}</h3><p class="muted">卡片渲染失败：${l(R.message)}</p></div>`}const H=e.hasModel||!!e.hasModelSO||!!e.previewable?'<span class="bm-badge ok">有模型</span>':'<span class="bm-badge warn" title="没有模型的夹心在汉堡里那一层看不见，且不会出现在汉堡工作台候选中">⚠ 无模型</span>';return`
