...
`;x(e=>{e==="layout"?location.href="/index.html#/layout":e==="manage"?location.href="/index.html#/manage":e==="dependencies"?location.href="/index.html#/dependencies":e==="custom-recipes"?location.href="/index.html#/custom-recipes":e==="guide"&&(location.href="/index.html#/guide")});let p=[],v=[];const b=new Map;let E="",m="all",u="all",I="all",B=!1,g="recipes",f=!0;const A=[20,40,60,80,100,120];function N(e,t){if(t==="all")return!0;const n=e??0;return t==="other"?!A.includes(n):n===t}function F(e){return String(e??"").replace(/·?DLC\d+/g,"").replace(/[（）()· ]/g,"")}function L(e){const t=/^dlc(\d+)_/.exec(e??"");return t?parseInt(t[1],10):0}function h(e){const t=new Map;for(const n of e){const l=F(n.nameZh??""),o=t.get(l);(!o||L(n.id)>L(o.id))&&t.set(l,n)}return[...t.values()]}function _(e){return j(e,{allRecipes:p,ingredientName:t=>{var n;return((n=b.get(t))==null?void 0:n.nameZh)??t},extraBadge:e.group==="levelset"?"本关":void 0})}function U(){const e=E.trim().toLowerCase();let t=p.filter(n=>!(!B&&n.intermediate||m!=="all"&&(n.type??"other")!==m||u!=="all"&&(n.group??"core")!==u||!N(n.score,I)||e&&![n.nameZh,n.nameEn??"",n.id,...(n.ingredients??[]).map(o=>{var s;return((s=b.get(o))==null?void 0:s.nameZh)??o})].join(" ").toLowerCase().includes(e)));return f&&(t=h(t)),t}function V(e){const t=e.group&&e.group!=="core"?` <span class="pc-badge">${a(y(e.group))}</span>`:"",n=e.nameEn&&e.nameEn.trim()?` <span class="muted pc-en">${a(e.nameEn)}</span>`:"";return`<div class="rl-ing-card" title="${a(e.id)}">
  ...
  </div>`}function z(){const e=E.trim().toLowerCase(),t=v.filter(s=>!(e&&!`${s.nameZh} ${s.nameEn??""} ${s.id}`.toLowerCase().includes(e)||u!=="all"&&(s.group??"core")!==u)),n=f?h(t):t;if(n.length===0)return'<div class="rl-empty">没有匹配的食材，试试调整搜索或筛选条件</div>';const l=new Map;for(const s of n){const i=s.group??"core";l.has(i)||l.set(i,[]),l.get(i).push(s)}return[...l.keys()].sort((s,i)=>{const r=c=>c==="core"?0:c==="levelset"?1:2;return r(s)-r(i)||s.localeCompare(i)}).map(s=>{const i=l.get(s);return`<section class="rl-section">
        <h2 class="rl-section-title">${a(y(s))}<span class="rl-section-count">${i.length}</span></h2>
        <div class="rl-ing-grid">${i.map(V).join("")}</div>
      </section>`}).join("")}function d(){const e=document.getElementById("rl-content"),t=document.getElementById("rl-types");t&&(t.style.display=g==="recipes"?"":"none");const n=document.getElementById("rl-score");if(n&&(n.style.display=g==="recipes"?"":"none"),g==="ingredients"){e.innerHTML=z();return}const l=U();if(l.length===0){e.innerHTML='<div class="rl-empty">没有匹配的菜谱，试试调整搜索或筛选条件</div>';return}e.innerHTML=w(l).map(([o,s])=>R(o,s.map(_).join(""),s.length)).join("")}function k(){const e=f?h(p.filter(r=>!r.intermediate)):p.filter(r=>!r.intermediate),t=w(e),n=document.getElementById("rl-types"),l=[{type:"all",label:"全部",count:e.length},...t.map(([r,c])=>({type:r,label:D(r),count:c.length}))];n.innerHTML=l.map(r=>`<button type="button" class="rl-chip-btn${r.type===m?" active":""}" data-type="${a(r.type)}">${a(r.label)}<span class="rl-cnt">${r.count}</span></button>`).join("");const o=document.getElementById("rl-group"),s=new Map;for(const r of e){const c=r.group??"core";s.set(c,(s.get(c)??0)+1)}const i=['<option value="all">全部来源</option>'];for(const[r,c]of s)i.push(`<option value="${a(r)}" ${r===u?"selected":""}>${a(y(r))} (${c})</option>`);o.innerHTML=i.join("")}function G(){document.getElementById("rl-search").addEventListener("input",e=>{E=e.target.value,d()}),document.getElementById("rl-types").addEventListener("click",e=>{const t=e.target.closest(".rl-chip-btn");t&&(m=t.dataset.type??"all",document.querySelectorAll(".rl-chip-btn").forEach(n=>n.classList.toggle("active",n===t)),d())}),document.getElementById("rl-group").addEventListener("change",e=>{u=e.target.value,d()}),document.getElementById("rl-score").addEventListener("change",e=>{const t=e.target.value;I=t==="all"?"all":t==="other"?"other":Number(t),d()}),document.getElementById("rl-intermediates").addEventListener("change",e=>{B=e.target.checked,d()}),document.getElementById("rl-web-reps").addEventListener("change",e=>{f=e.target.checked,k(),d()}),document.querySelectorAll(".rl-view-btn").forEach(e=>{e.addEventListener("click",()=>{g=e.dataset.view??"recipes",document.querySelectorAll(".rl-view-btn").forEach(t=>t.classList.toggle("active",t===e)),d()})})}async function K(){try{const[l,o]=await Promise.all([H(""),S()]);p=l,v=o}catch(l){Z(l);return}for(const l of v)b.set(l.id,l);const e=await T().catch(()=>!1),t=p.filter(l=>!l.intermediate).length,n=h(p.filter(l=>!l.intermediate)).length;$(`共 ${p.length} 个菜谱（成品 ${t} · Web去重后 ${n}）${e?"":" · 静态数据"}`),k(),d(),G()}K();
