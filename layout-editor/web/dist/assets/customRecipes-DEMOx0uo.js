import{a as ue,n as pe,w as ye,a0 as q,a1 as A,j as be,a2 as ge,a3 as ve,a4 as fe,a5 as $e,a6 as he,a7 as Ie,a8 as Ee,f as we,a9 as ne,aa as ke}from"./recipeCard-CRTxsqII.js";import{h as k,s as x,o as R,c as h}from"./main-CugeIIZL.js";function n(l){return String(l??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const H=/^[A-Za-z_][A-Za-z0-9_]*$/;function g(l,e=!0){const a=document.getElementById("cr-status");a&&(a.textContent=l,a.classList.toggle("err",!e),a.classList.toggle("ok",e&&l.length>0))}function _(l,e){return document.body.classList.add("manage-bg"),l.innerHTML=`
    ${pe("custom-recipes")}
  ...
  `,ye(a=>{a==="layout"?(location.hash="#/layout",location.reload()):a==="manage"?(location.hash="#/manage",location.reload()):a==="recipes"&&(location.href="/recipes")}),document.getElementById("cr-content")}async function Be(l){const e=_(l,"自定义菜谱管理");Z("加载关卡集…");let a=[];try{a=await ue()}catch(c){z(c);return}g(`共 ${a.length} 个关卡集`),e.innerHTML=`
  ...
  `,e.querySelectorAll("[data-open]").forEach(c=>c.addEventListener("click",()=>void j(l,c.dataset.open)))}function Z(l){const e=document.getElementById("cr-content");e&&(e.innerHTML=`<p class="muted">${n(l)}</p>`),g(l)}function z(l){const e=l instanceof Error?l.message:String(l);g(e,!1);const a=document.getElementById("cr-content");a&&(a.innerHTML=`<div class="m-block"><h3>出错</h3><p>${n(e)}</p></div>`)}async function j(l,e){var B,D;const a=_(l,`自定义菜谱 · ${n(e)}`);Z("加载菜谱配置…");let c,p;try{c=await q(e),p=await A(e)}catch(d){z(d);return}g(`${p.length} 个菜谱 · UID前缀：${c.uidPrefix}`);const u=c.categories??[];function s(d){return d.zh||d.id}let i="";function t(d){const w=d?p.filter(r=>r.category===d):p;return w.length===0?'<p class="muted">该分类下暂无菜谱</p>':`<div class="m-grid">${w.map(r=>`
  ...
  `;function I(){(async()=>{x("加载…");try{c=await q(e),p=await A(e)}catch(d){z(d);return}finally{k()}g(`${p.length} 个菜谱 · UID前缀：${c.uidPrefix}`),document.getElementById("cr-sidebar").innerHTML=f(),E(),document.getElementById("cr-grid").innerHTML=t(i),L()})()}function E(){var d,w;document.querySelectorAll(".cr-cat-item").forEach(r=>{r.addEventListener("click",()=>{i=r.dataset.cat??"",document.getElementById("cr-sidebar").innerHTML=f(),E(),document.getElementById("cr-grid").innerHTML=t(i),L()})}),(d=document.getElementById("cr-new-cat"))==null||d.addEventListener("click",()=>Se(e,I)),(w=document.getElementById("cr-manage-cat"))==null||w.addEventListener("click",()=>Le(e,c.categories,I))}function L(){document.querySelectorAll("[data-edit]").forEach(d=>d.addEventListener("click",()=>void te(l,e,d.dataset.edit))),document.querySelectorAll("[data-del]").forEach(d=>d.addEventListener("click",()=>xe(l,e,d.dataset.del,I)))}E(),L(),(B=document.getElementById("cr-back"))==null||B.addEventListener("click",()=>void Be(l)),(D=document.getElementById("cr-new-recipe"))==null||D.addEventListener("click",()=>void te(l,e,null))}async function te(l,e,a){var G,N,V,J,X;const c=a!=null,p=_(l,c?"编辑菜谱":"新建菜谱");Z("加载参考数据…");let u=[],s,i,t;try{[u,s,i]=await Promise.all([be().catch(()=>[]),ge(e),q(e)]),c&&(t=(await A(e)).find(m=>m.assetPath===a))}catch(o){z(o),k();return}k();const f=!c,I=(t==null?void 0:t.recipeName)??"",E=(t==null?void 0:t.nameZh)??"",L=(t==null?void 0:t.nameEn)??"",B=(t==null?void 0:t.category)??(((G=i.categories)==null?void 0:G.length)>0?i.categories[0].id:""),D=(t==null?void 0:t.score)??0,d=(t==null?void 0:t.type)??"Cooked",w=(t==null?void 0:t.compositionIds)??[],r=(t==null?void 0:t.cookingStepId)??"",ce="",oe=(t==null?void 0:t.platingStepId)??"",le="",se="",U=i.categories??[];function ie(o){return o.zh||o.id}function de(){let o=U.map(m=>`<option value="${n(m.id)}" ${m.id===B?"selected":""}>${n(ie(m))}</option>`).join("");return!U.some(m=>m.id===B)&&B&&(o+=`<option value="${n(B)}" selected>${n(B)}</option>`),`<select id="cr-type-cat" class="m-select">${o}</select>`}function M(o,m,v){const b=new Map;for(const y of o)b.has(y.id)||b.set(y.id,y.nameZh||y.id);return`<select id="${v}" class="m-select">
      <option value="">— 不设置 —</option>
      ${[...b.entries()].map(([y,$])=>`<option value="${n(y)}" ${y===m?"selected":""}>${n($)} (${n(y)})</option>`).join("")}
    </select>`}function re(o,m){const v=o.group&&o.group!=="core"?` <span class="pc-badge">${we(o.group)}</span>`:"",b=o.nameEn&&o.nameEn.trim()||"";return`<label class="pick-card${m?" selected":""}" data-guid="${o.guid}">
      <input type="checkbox" value="${n(o.id)}" data-ingid="${n(o.id)}" ${m?"checked":""}>
      <span class="pc-head"><img class="food-icon" src="/icons/ingredients/${n(o.id)}.png" alt="" onerror="this.src='/icons/_placeholder.png'" /><span class="pc-name">${n(o.nameZh)}${v}${b?` <span class="muted pc-en">${b}</span>`:""}</span></span>
  ...
  `;function F(){var $;const o=document.getElementById("cr-type").value,m=document.getElementById("cr-cook-row"),v=($=document.getElementById("cr-cook-step"))==null?void 0:$.closest(".m-field"),b=document.getElementById("cr-mix-row"),y=document.getElementById("cr-mix-prog-row");m&&(m.style.display=o==="Composite"?"none":""),v&&(v.style.display=o==="Composite"?"none":""),b&&(b.style.display=o==="Mixed"?"":"none"),y&&(y.style.display=o==="Mixed"?"":"none")}(N=document.getElementById("cr-type"))==null||N.addEventListener("change",F),F(),(V=document.getElementById("cr-new-cat-inline"))==null||V.addEventListener("click",()=>Ce(e,async()=>{i=await q(e);const o=document.getElementById("cr-type-cat");if(o){const m=i.categories[i.categories.length-1];m&&(o.innerHTML=i.categories.map(v=>`<option value="${n(v.id)}">${n(v.zh||v.id)}</option>`).join(""),o.value=m.id)}})),(J=document.getElementById("cr-form-back"))==null||J.addEventListener("click",()=>void j(l,e)),(X=document.getElementById("cr-form-save"))==null||X.addEventListener("click",async()=>{var v,b,y,$,K,Q,W,Y,ee;const o=document.getElementById("cr-rec-name").value.trim();if(!o)return alert("请填写标识符");if(!H.test(o))return alert("标识符仅允许英文字母/数字/下划线，且不能以数字开头");const m={setName:c?void 0:e,assetPath:c?a:void 0,recipeName:o,nameZh:document.getElementById("cr-zh").value.trim(),nameEn:document.getElementById("cr-en").value.trim(),category:document.getElementById("cr-type-cat").value,score:Number(document.getElementById("cr-score").value)||0,type:document.getElementById("cr-type").value,compositionIds:[],cookingStepId:((v=document.getElementById("cr-cook-step"))==null?void 0:v.value)??"",cookingStepIconId:((b=document.getElementById("cr-cook-icon"))==null?void 0:b.value)??"",platingStepId:((y=document.getElementById("cr-plate-step"))==null?void 0:y.value)??"",mixingIconId:(($=document.getElementById("cr-mix-icon"))==null?void 0:$.value)??"",modelPrefabId:((K=document.getElementById("cr-model-ref"))==null?void 0:K.value)??"",cookingProgress:Number(((Q=document.getElementById("cr-cook-prog"))==null?void 0:Q.value)??"1")||1,mixingProgress:Number(((W=document.getElementById("cr-mix-prog"))==null?void 0:W.value)??"1")||1};document.querySelectorAll("#cr-ing-grid input:checked").forEach(S=>{const C=S.dataset.ingid;C&&m.compositionIds.push(C)}),x("保存中…");try{c?await ve(m):await fe(m);const S=a||`Assets/LevelSets/${e}/custom_recipes/${m.category}/${o}.asset`,C=(Y=document.getElementById("cr-icon-upload").files)==null?void 0:Y[0],T=(ee=document.getElementById("cr-model-upload").files)==null?void 0:ee[0];if(C){const P=await O(C);await $e(e,S,C.name,P)}if(T){const P=await O(T);await he(e,S,T.name,P)}g(c?"已更新菜谱":"已创建菜谱"),j(l,e)}catch(S){g(S.message,!1)}finally{k()}});function O(o){return new Promise((m,v)=>{const b=new FileReader;b.onload=()=>{const y=b.result,$=y.indexOf(",");m($>=0?y.substring($+1):y)},b.onerror=v,b.readAsDataURL(o)})}}function xe(l,e,a,c){var u,s;const p=a.split("/").pop()??a;R(`删除菜谱 · ${n(p)}`,"<p>将永久删除菜谱资源及其模型文件夹，且<b>不可恢复</b>。</p>",'<button type="button" class="m-btn" data-cancel>取消</button><button type="button" class="m-btn danger" data-ok>确认删除</button>'),(u=document.querySelector("[data-cancel]"))==null||u.addEventListener("click",h),(s=document.querySelector("[data-ok]"))==null||s.addEventListener("click",async()=>{x("删除中…");try{await Ee(a),h(),g("已删除菜谱"),c()}catch(i){g(i.message,!1)}finally{k()}})}function Se(l,e){R("新建分类",`<label class="m-field">分类ID（仅字母/数字/下划线，用于目录名）<input type="text" id="cr-cat-id" placeholder="MyCategory"></label>
     <label class="m-field">中文名<input type="text" id="cr-cat-zh" placeholder="我的分类"></label>
     <label class="m-field">英文名<input type="text" id="cr-cat-en" placeholder="My Category"></label>`,'<button type="button" class="m-btn" data-cancel>取消</button><button type="button" class="m-btn primary" data-ok>创建</button>'),ae(h,async()=>{const a=document.getElementById("cr-cat-id").value.trim();if(!a)return alert("请填写分类ID");if(!H.test(a))return alert("分类ID仅允许英文字母/数字/下划线");const c=document.getElementById("cr-cat-zh").value.trim(),p=document.getElementById("cr-cat-en").value.trim();x("创建分类…");try{await ne(l,a,c||a,p||a),h(),g("已创建分类"),e()}catch(u){g(u.message,!1)}finally{k()}})}function Ce(l,e){const a=prompt("分类ID（仅字母/数字/下划线）：");if(!a||!H.test(a)){alert("ID非法");return}const c=prompt("分类中文名：")||a,p=prompt("分类英文名：")||a;x("创建分类…"),ne(l,a,c,p).then(()=>{g("已创建分类"),e()}).catch(u=>g(u.message,!1)).finally(()=>k())}function Le(l,e,a){var u;function c(s){return`${s.zh||s.id}${s.en?` (${s.en})`:""}`}const p=e.map(s=>`
